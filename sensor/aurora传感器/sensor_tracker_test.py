...
class NDISensorTracker:
    ...
    def get_tx_transforms(self) -> Dict[int, Dict]:
        ...
        try:
            ...
            for i in range(num_handles):
                if pos + 2 > len(data):
                    # print(f"  句柄 {i}: 数据不足，位置 {pos}, 长度 {len(data)}")
                    break
    ...
    def run_tracking_loop(self):
        ...
        try:
            while True:
                ...
                # 每10帧检查一次传感器状态
                if frame_count - last_status_check >= 10:
                ...
                if transforms:
                    ...
                    for handle, transform in transforms.items():
                        ...
                        print(f"传感器 {physical_port} (句柄 {handle:02X}):")
                        if transform.get('valid', False):
                            has_valid_data = True
                            pos = transform.get('translation', {})
                            rot = transform.get('rotation', {})
                            error = transform.get('error', 0.0)
                            frame = transform.get('frame_number', 0)
                            print(f"  位置: X={pos.get('x', 0):7.3f} mm, Y={pos.get('y', 0):7.3f} mm, Z={pos.get('z', 0):7.3f} mm")
                            print(f"  旋转: q0={rot.get('q0', 0):7.4f}, qx={rot.get('qx', 0):7.4f}, "
                                  f"qy={rot.get('qy', 0):7.4f}, qz={rot.get('qz', 0):7.4f}")
                            print(f"  误差: {error:.4f}, 帧号: {frame}")
                        else:
                            status = transform.get('status', 'UNKNOWN')
                            print(f"  状态: {status}")
                    
                    if not has_valid_data:
                        print("  警告: 没有有效的坐标数据，请检查传感器是否已正确启用")
                    
                    print("=" * 60)
                else:
                    if frame_count % 20 == 0:  # 每20帧显示一次提示
                        print(f"[帧 {frame_count}] 等待坐标数据...")
...
def main():
    ...
    print("=" * 50)
    print("NDI电磁传感器跟踪器")
    print("基于Source目录中的CombinedAPI示例代码")
    print("=" * 50)
    
    # 创建跟踪器实例（可以设置debug=True启用调试模式）
    # 建议先启用调试模式查看详细信息
    tracker = NDISensorTracker(port="COM9", baudrate=9600, debug=True)
    ...
    try:
        ...
        # 显示当前传感器状态
        print("\n当前传感器状态:")
        tracker.print_sensor_status()
        
        # 询问用户要启用哪些传感器
        print("\n请输入要启用的传感器编号（用逗号分隔，例如: 1,2 或直接回车跳过）:")
        try:
            ...
            if user_input:
                ...
                if port_numbers:
                    ...
                    print("未输入有效的端口号，跳过启用步骤")
            else:
                print("未输入端口号，跳过启用步骤")
        except (ValueError, KeyboardInterrupt):
            print("输入无效或取消，跳过启用步骤")
